...
def load_data(model_args):
    ...
    return train_cf, test_cf, user_dict, n_params, graph, [adj_mat_list, norm_mat_list, mean_mat_list]   #   """这是一个包含三个稀疏矩阵列表的集合，用于表示知识图谱的关系图。 adj_mat_list：原始的邻接矩阵列表，表示不同关系的连接。norm_mat_list：双向归一化的邻接矩阵列表，用于图神经网络的传播。mean_mat_list：单向归一化的邻接矩阵列表，也用于图神经网络的传播.这些矩阵通过 build_sparse_relational_graph 函数生成。
