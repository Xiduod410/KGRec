...
from modules.contrast import Contrast
...
class CausalRationaleModule(nn.Module):
    def __init__(self, embedding_dim):
        super(CausalRationaleModule, self).__init__()
        self.user_pref_layer = nn.Linear(embedding_dim * 2, embedding_dim)

    def forward(self, attr_embed, structure_embed):
        x = torch.cat([attr_embed, structure_embed], dim=-1)
        u_pref = self.user_pref_layer(x)
        return u_pref

    def causal_score(self, u_pref, item_embed, attr_embed):
        g_ui = torch.sum(u_pref * item_embed, dim=-1)
        h_ia = torch.sum(item_embed * attr_embed, dim=-1)
        score = g_ui * torch.sigmoid(h_ia)
        return score

    def counterfactual_score(self, item_embed, attr_embed):
        """
        在反事实世界中，去掉 A（属性干预）或结构干预，用 item 表示和 attr 表示单独预测
        """
        h_ia = torch.sum(item_embed * attr_embed, dim=-1)
        return torch.sigmoid(h_ia)  # 仅保留 A→S 路径，模拟无偏干预打分
...
class TestKGRec(nn.Module):
    def __init__(self, data_config, args_config, graph, adj_mat, hp_dict=None):
        ...
        self.causal_module = CausalRationaleModule(self.emb_size)
    ...
    def forward(self, batch=None):
        ...
        # --- Node dropout and attention edge masking ---
        ...
        enc_edge_index, enc_edge_type, masked_edge_index, masked_edge_type, mask_bool = \
            _mae_edge_mask_adapt_mixed(edge_index, edge_type, topk_attn_edge_id)
        ...
        # --- Causal Rationale 分支: 提前计算所有所需变量 ---
        with torch.no_grad():
            pos_neighbors = enc_edge_index[1][torch.isin(enc_edge_index[0], pos_item)]
            item_attr_embed = scatter_mean(entity_gcn_emb[pos_neighbors], pos_neighbors, dim=0, dim_size=self.n_entities)

            user_interacted = entity_gcn_emb[pos_item]
            user_attr_embed = scatter_mean(user_interacted, user, dim=0, dim_size=self.n_users)

        structure_embed = item_attr_embed[pos_item]  # 正例物品结构表示
        attr_embed = user_attr_embed[user]  # 用户历史属性表示
        item_embed = entity_gcn_emb[pos_item]  # 正例物品表示
        u_pref = self.causal_module(attr_embed, structure_embed)

        causal_score_pos = self.causal_module.causal_score(u_pref, item_embed, attr_embed)

        # --- BPR Loss with Causal Score ---
        loss, rec_loss, reg_loss = self.create_bpr_loss(
            u_e, pos_e, neg_e,
            pos_scores=causal_score_pos,
            use_causal=True
        )
        ...
        # --- 反事实打分缓存（用于评估）
        counterfactual_scores = self.causal_module.counterfactual_score(item_embed, attr_embed)
        self.last_counterfactual_score = counterfactual_scores
    ...
    def rating(self, u_g_embeddings, i_g_embeddings, mode='default'):
        ...
        mode:
          ...
          - 'counterfactual': 使用反事实评分
        """
        if mode == 'default':
            return torch.matmul(u_g_embeddings, i_g_embeddings.t())  # [B, N]

        elif mode == 'causal':
            batch_size = u_g_embeddings.shape[0]
            item_num = i_g_embeddings.shape[0]

            # [B, N, D]：为每个用户复制 item，构造结构信息
            structure_embed = i_g_embeddings.unsqueeze(0).expand(batch_size, item_num, -1)
            attr_embed = u_g_embeddings.unsqueeze(1).expand(batch_size, item_num, -1)

            # 生成因果偏好表示 [B, N, D]
            u_pref = self.causal_module(attr_embed, structure_embed)

            # 使用 causal_score 获取打分 [B, N]
            score = self.causal_module.causal_score(u_pref, structure_embed, attr_embed)
            return score
            ...
            # [B, N, D]：反事实打分也需要扩展维度
            attr_embed = u_g_embeddings.unsqueeze(1).expand(batch_size, item_num, -1)
            item_embed = i_g_embeddings.unsqueeze(0).expand(batch_size, item_num, -1)

            score = self.causal_module.counterfactual_score(item_embed, attr_embed)  # [B, N]
            return score
    ...
    def create_bpr_loss(self, users, pos_items, neg_items, pos_scores=None, use_causal=False):
        """
        users, pos_items, neg_items: [B, D] embedding 向量，或[batch] id 映射
        pos_scores: 如果 use_causal=True，则传入正例的 causal 打分
        """
        if use_causal:
            if pos_scores is None:
                raise ValueError("Causal mode requires pos_scores input.")
            # 对于负例，我们这里仍使用传统打分方式（u * i），因为 causal_score_neg 未预先计算
        ...
        # BPR loss
        mf_loss = -1 * torch.mean(torch.nn.functional.logsigmoid(pos_scores - neg_scores))

        # 正则项
        regularizer = (users.norm(p=2) ** 2 + pos_items.norm(p=2) ** 2 + neg_items.norm(p=2) ** 2) / 2
