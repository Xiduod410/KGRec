...
class AttnHGCN(nn.Module):
    ...
    def shared_layer_agg(self, user_emb, entity_emb, edge_index, edge_type, inter_edge, inter_edge_w, relation_emb):
        ...
        relation_vec_for_attn = relation_emb[edge_type - 1].view(-1, self.n_heads, self.d_k)
        key = key * relation_vec_for_attn

        edge_attn_score = (query * key).sum(dim=-1) / math.sqrt(self.d_k)

        # ✅ 修复这段原始爆错部分
        relation_ids = edge_type - 1
        relation_vec = relation_emb[relation_ids]  # [n_edges, dim]
        neigh_relation_emb = entity_emb[tail] * relation_vec  # ✅ 对齐
