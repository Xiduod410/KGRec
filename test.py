...
if __name__ == '__main__':
    try:
        ...
        # 在训练结束后，保存模型权重
        logger.info("Training complete. Saving model to %s" % save_path)
        torch.save(model.state_dict(), save_path)
        ...
        # 获取合理性分数
        edge_attn_score, edge_index, edge_type = model.generate_global_attn_score()

        # 将合理性分数转换为numpy数组
        rationality_scores = edge_attn_score.detach().cpu().numpy()

        # 绘制分布图
        plt.figure(figsize=(10, 6))
        sns.histplot(rationality_scores, bins=30, kde=True, color='blue')
        plt.title('Distribution of Rationality Scores in KGRec', fontsize=16)
        plt.xlabel('Rationality Score', fontsize=12)
