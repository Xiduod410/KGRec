...
if __name__ == '__main__':
    try:
        ...
        # # 如果 log_fn 未指定，则根据数据集名称、模型名称和时间戳生成一个唯一的日志文件名
        if args.log_fn is None and args.log:
            timestamp = time.strftime("%Y%m%d-%H%M%S")
            args.log_fn = f"{args.dataset}-{args.model}-{timestamp}.log"

        # # 打印日志文件名（用于调试）
        # print(f"Log file name: {args.log_fn}")
            ...
            with tqdm(total=len(train_cf) // args.batch_size) as pbar:
                while s + args.batch_size <= len(train_cf):
                    batch = get_feed_dict(train_cf_with_neg,
                                          s, s + args.batch_size)
                    # print(batch) # 看看batch里都有啥，找到对应pos_neighbors的数据键 # 假设 batch 是一个字典, key 叫 'neighbors'
                    # print("Max ID from get_feed_dict:", batch['users'].max().item()) #
                    # print("Min ID from get_feed_dict:", batch['users'].min().item()) # 别忘了检查一下 self.n_items 在模型初始化时是多少 #
                    # print("Model's n_items:", model.n_items)
