...
setproctitle.setproctitle('EXP@KGRec-Causal')
...
# # Function to create feed dictionary for the model
...
if __name__ == '__main__':
    try:
        ...
        """define model"""
        model = TestKGRec(n_params, args, graph, mean_mat_list[0]).to(device)
        model.print_shapes()
        ...
        epoch_list, recall_list, ndcg_list = [], [], []
        ...
        for epoch in range(args.epoch):
            ...
            if epoch % test_interval == 0 and epoch >= 1:
                ...
                train_res.field_names = ["Epoch", "training time", "testing time", "Loss", "recall", "ndcg",
                                         "precision", "hit_ratio"]
                train_res.add_row(
                ...
                recall_list.append(ret['recall'][0])
                ndcg_list.append(ret['ndcg'][0])
                ...
                if ret['recall'][0] == cur_best_pre_0 and args.save:
                    save_path = args.out_dir + log_fn + '_causal.ckpt'
        ...
        plt.subplot(1, 2, 1)
        plt.plot(epoch_list, recall_list, marker='o')
        plt.title('Recall@20 vs Epoch')
        plt.xlabel('Epoch')
        plt.ylabel('Recall@20')

        plt.subplot(1, 2, 2)
        plt.plot(epoch_list, ndcg_list, marker='o')
        plt.title('NDCG@20 vs Epoch')
        plt.xlabel('Epoch')
        plt.ylabel('NDCG@20')

        plt.tight_layout()
        save_path = os.path.join(save_dir, "training_metrics_causal.png")
